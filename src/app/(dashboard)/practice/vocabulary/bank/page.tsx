...
export default function VocabularyBankPage() {
  ...
  const toggleBookmark = async (vocabId: string, currentStatus: boolean) => {
    ...
    setVocabList((prev) =>
      prev.map((v) => (v.id === vocabId ? { ...v, isBookmarked: !currentStatus } : v))
    );
  ...
  const filteredList = vocabList.filter((v) => {
    ...
    if (searchQuery) {
      return (
        v.word.word.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Review your learned words and track your mastery.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="relative max-w-md flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border-gray-200 bg-gray-50 py-2.5 pr-4 pl-10 text-sm focus:border-green-500 focus:ring-green-500"
          />
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setFilter("all")}
            className={`rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${filter === "all" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
          >
            All Words
          </button>
          <button
            onClick={() => setFilter("bookmarked")}
            className={`rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${filter === "bookmarked" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
          >
            Bookmarked
          </button>
          <button
            onClick={() => setFilter("mastered")}
            className={`rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${filter === "mastered" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
          >
            Mastered
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredList.map((v) => (
            <div
              key={v.id}
              className="group relative flex flex-col justify-between rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-shadow hover:shadow-md"
            >
              <div>
                ...
                <p className="mt-4 line-clamp-2 text-sm text-gray-700">{v.word.meaning}</p>
                <p className="mt-2 line-clamp-2 text-xs text-gray-500 italic">
                  &quot;{v.word.exampleSentence}&quot;
                </p>
              </div>
              ...
              <div className="mt-6 flex items-center justify-between border-t border-gray-50 pt-4">
                <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    <div
                      key={i}
                      className={`h-1.5 w-4 rounded-full ${i < v.masteryLevel ? "bg-green-500" : "bg-gray-100"}`}
                    />
                  ))}
                </div>
                {v.masteryLevel === 5 && (
