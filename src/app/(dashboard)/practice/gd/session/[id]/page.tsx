    ...
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-6xl flex-col gap-6 lg:flex-row">
      ...
      <div className="flex flex-1 flex-col gap-4">
        <div className="flex items-center justify-between rounded-2xl border border-gray-100 bg-white px-6 py-4 shadow-sm">
          <div>
            <h2 className="max-w-[400px] truncate text-lg font-bold text-gray-900">
              {session.topic}
            </h2>
          ...
          <div className="flex items-center gap-4">
            <div
              className={`flex items-center gap-2 rounded-lg px-3 py-1.5 font-mono text-sm font-bold ${timeLeft < 60 ? "bg-red-50 text-red-600" : "bg-gray-50 text-gray-600"}`}
            >
        ...
        <div className="grid flex-1 auto-rows-fr grid-cols-2 gap-4">
          ...
          {participants.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`relative flex flex-col items-center justify-center rounded-3xl border bg-gray-50 p-6 transition-all duration-300 ${isSpeaking ? "border-4 border-blue-500 shadow-xl ring-4 shadow-blue-100 ring-blue-50" : "border-gray-200"}`}
              >
                {isSpeaking && (
          ...
          {/* User Participant */}
          <div
            className={`relative flex flex-col items-center justify-center rounded-3xl border bg-gray-50 p-6 transition-all duration-300 ${isRecording ? "border-4 border-green-500 shadow-xl shadow-green-100" : "border-gray-200"}`}
          >
      ...
      <div className="flex w-full flex-col rounded-2xl border border-gray-100 bg-white shadow-sm lg:w-96">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          {messages.map((m) => (
            <div
              key={m.id}
              className={`flex flex-col ${m.speakerRole === "user" ? "items-end" : "items-start"}`}
            >
                ...
                className={`max-w-[85%] rounded-2xl px-4 py-2 text-sm ${
                  m.speakerRole === "user"
                    ...
                    : m.speakerRole === "moderator"
                      ? "rounded-tl-none bg-gray-800 text-white"
          ...
          {transcript && (
            <div className="flex flex-col items-end">
              ...
              <div className="max-w-[85%] rounded-2xl rounded-br-none bg-blue-600/60 px-4 py-2 text-sm text-white italic">
                {transcript}
              </div>
            </div>
          )}
          ...
          {isAiThinking && (
            <div className="flex flex-col items-start">
              <div className="flex items-center gap-2 rounded-2xl rounded-tl-none bg-gray-50 px-4 py-3 text-sm text-gray-500">
        ...
        <div className="border-t border-gray-100 p-4">
          {!isRecording ? (
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-red-500 py-3 text-sm font-bold text-white shadow-[0_0_15px_rgba(239,68,68,0.5)] transition-all hover:bg-red-600"
            >
          ...
          <p className="mt-2 text-center text-[10px] text-gray-400">
            If you have nothing to say, just tap speak and stop immediately to pass the turn to the
            AI.
          </p>
        </div>
      </div>
