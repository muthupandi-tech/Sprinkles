  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50"
      >
        {isGenerating ? (
      ...
      <div
        id="report-content"
        className="absolute left-[-9999px] top-[-9999px] w-[800px] bg-white p-12 text-gray-900"
        style={{ display: "none" }}
      >
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-4 rounded-xl bg-gray-50 p-6">
            <div>
              ...
              <p className="text-3xl font-bold text-blue-600">{progress.overallScore.toFixed(0)}/100</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">{progress.vocabularyScore.toFixed(0)}</p>
            </div>
          </div>
        </div>
          ...
          <ul className="space-y-3 pl-5 list-disc text-gray-700">
            {recommendations.length > 0 ? (
              recommendations.map((rec, i) => (
                <li key={i} className="leading-relaxed">{rec.content}</li>
              ))
            ) : (
