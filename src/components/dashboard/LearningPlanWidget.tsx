  ...
  if (!plan) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-blue-200 bg-blue-50/50 p-6 text-center shadow-sm">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-1">Daily Learning Plan</h3>
        <p className="text-sm text-gray-500 mb-4 max-w-[250px]">Get a personalized daily practice plan from your AI Coach.</p>
        <button
      ...
      <div className="space-y-3">
        {plan.tasksJson.map(task => (
          ...
          >
            ...
            <span className={`text-sm text-left transition-colors ${task.completed ? "text-gray-400 line-through" : "text-gray-700 font-medium"}`}>
              {task.title}
            </span>
          </button>
        ))}
